...
const AFFILIATION_RE = /\b(?:university|universit[aäeé]|institute|institut|department|dept\.?|laboratory|laboratories|lab\.?|research center|research centre|school of|faculty of|college|corporation|corp\.?|inc\.?|gmbh|email)\b/i;
const HEADER_RE = /^(?:journal\b|available online\b|www\.|https?:\/\/|doi\s*:|received\b|accepted\b|copyright\b|©|preprint\b)/i;
...
function trimIdentifierPunctuation(value) {
  return clean(value).replace(/[.,;:]+$/g, "").replace(/\)+$/g, (closing) => closing.length ? "" : closing);
}
