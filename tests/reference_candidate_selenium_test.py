...
def main():
    ...
    try:
        ...
        assert_true(
            ...
            "Identifier-less reference should explain its matching evidence",
        )
        ...
        canonical_before = driver.execute_script(
            "return arguments[0].__paperMapReference.resolution.canonical;",
            rows[2],
        )
        ...
        wait.until(
            lambda d: d.find_elements(By.CSS_SELECTOR, "#pdf-reference-list .pdf-reference-row")[2]
            .get_attribute("data-resolution-status") == "matched"
        )
        rows = driver.find_elements(By.CSS_SELECTOR, "#pdf-reference-list .pdf-reference-row")
        assert_true(
            "Selected explicitly from resolver candidates" in rows[2].text,
            "Chosen metadata candidate should record explicit review",
        )
        ...
        assert_true(
            selected["canonical"]["semanticScholarId"] == "2222222222222222222222222222222222222222",
            "The explicitly chosen canonical candidate should persist",
        )
        assert_true(
            references[2]["rawText"].startswith("Linus Author"),
            "Original raw bibliography evidence should remain alongside the canonical selection",
        )
