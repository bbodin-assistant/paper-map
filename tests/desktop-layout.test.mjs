...
test("desktop layout keeps the requested controls and floating map affordances", async () => {
  ...
  assert.match(css, /\.desktop-toolbar-left\s*\{[\s\S]*?order: 0;/);
  assert.match(css, /\.toolbar-actions\.desktop-add-actions\s*\{[\s\S]*?order: 1;[\s\S]*?justify-content: flex-end;[\s\S]*?margin-left: auto;/);
  assert.match(css, /\.desktop-toolbar-right\s*\{[\s\S]*?order: 2;[\s\S]*?margin-left: auto;/);
